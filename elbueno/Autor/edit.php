...
if (isset($_GET['idAutor'])) {
    ...
    $stm=$conexion->prepare("SELECT *FROM autor where idAutor=:txtid");
 ...
 if ($_POST) {
    ...
    $stm=$conexion->prepare("UPDATE autor SET nombre=:nombre,aPaterno=:aPaterno,aMaterno=:aMaterno,calle=:calle,colonia=:colonia,cuidad=:cuidad,estado=:estado,telefono=:telefono,pais=:pais WHERE idAutor =:txtid ");
    $stm->bindParam(":txtid",$txtid);
